...
var jobNumberSchema2 = stringType().regex(/^\d{5}-\d{0,8}$/, "jobNumber format invalid.").brand("jobNumber");
var companyNameSchema2 = stringType().brand("companyName");
var homePageSchema2 = stringType().url("home page should be url").brand("homePage");
var occupationSchema2 = stringType().min(1, "occupation should not be empty.").brand("occupation");
var employmentTypeSchema2 = enumType(["\u6B63\u793E\u54E1", "\u30D1\u30FC\u30C8\u52B4\u50CD\u8005", "\u6B63\u793E\u54E1\u4EE5\u5916", "\u6709\u671F\u96C7\u7528\u6D3E\u9063\u52B4\u50CD\u8005"]).brand("employmentType");
var RawReceivedDateShema2 = stringType().regex(
  /^\d{4}年\d{1,2}月\d{1,2}日$/,
  "received date format invalid. should be yyyy\u5E74mm\u6708dd\u65E5"
).brand("receivedDate(raw)");
var RawExpiryDateSchema2 = stringType().regex(
  /^\d{4}年\d{1,2}月\d{1,2}日$/,
  "expiry date format invalid. should be yyyy\u5E74mm\u6708dd\u65E5"
).brand("expiryDate(raw)");
var RawWageSchema2 = stringType().min(1, "wage should not be empty").brand("wage(raw)");
var RawWorkingHoursSchema2 = stringType().min(1, "workingHours should not be empty.").brand("workingHours(raw)");
var workPlaceSchema2 = stringType().brand("workPlace");
var jobDescriptionSchema2 = stringType().brand("jobDescription");
var qualificationsSchema2 = stringType().brand("qualifications");
var RawEmployeeCountSchema2 = stringType().brand("employeeCount(raw)");
var r = Symbol();
var e = Symbol();
var ec = Symbol();
var transformedReceivedDateSchema = RawReceivedDateShema2.transform(
  (value) => {
...
var transformedExpiryDateSchema = RawReceivedDateShema2.transform(
  (value) => {
    const dateStr = value.replace("\u5E74", "-").replace("\u6708", "-").replace("\u65E5", "");
    const isoDate = new Date(dateStr).toISOString();
    return isoDate;
...
var transformedWageSchema = RawWageSchema2.transform((value) => {
  ...
  (wage) => zod_default.object({ wageMin: zod_default.number(), wageMax: zod_default.number() }).parse(wage)
);
var transformedWorkingHoursSchema = RawWorkingHoursSchema2.transform(
  (value) => {
...
var transformedEmployeeCountSchema = RawEmployeeCountSchema2.transform(
  (val, ctx) => {
    const match = val.match(/\d+/);
    if (!match) {
      ctx.addIssue({
        code: zod_default.ZodIssueCode.custom,
...
var D1Client = class {
  ...
  insertJob(job) {
    return this.checkDuplicate(job.jobNumber).andThen((duplicated) => {
      if (duplicated) {
        return err({
          type: "DuplicateJobError",
