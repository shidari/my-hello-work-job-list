...
- **クローリング処理時間**: 200件/45秒（約4.5件/秒）
- **スケーラビリティ対応**: 10万件規模時はEC2移行を検討（Lambda料金制約）
- **API設計**: JWTベースページネーション、15分有効期限、カーソルベース実装

**技術的課題と解決実績**:

- **Effect-ts学習**: Chunk概念が最難関、create/running effectも初期困難
- **セッション管理**:
  ハローワークサイト制約でheadless-browserによる段階的遷移が必須
- **技術的負債**:
  headless-crawlerのEffect-ts密結合をインターフェースレベルで分離予定
...
- **技術的課題と解決策**:
  - **セッション管理の問題**:
    最初はCloudflareで軽量なパーサーを使ってクローリングしようとしたが、ハローワークはセッションの関係でfetchができず、headless-browser（Playwright）での実装が必要。ホームから順々にクリックして進まないと情報取得ができない制約が最も困難
  - **2段階処理の実装**:
